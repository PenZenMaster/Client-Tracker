...
# Create a new workbook and rename the default sheet to "Tasks"
wb = Workbook()
ws_tasks = wb.active
ws_tasks.title = "Tasks"

# Set up headers for the Tasks sheet
headers = ["Task", "Subtasks", "Timeline", "Status", "Notes"]
ws_tasks.append(headers)

# Add sample data rows to the Tasks sheet
tasks_data = [
...
# Create the Lists sheet to store dropdown options
ws_lists = wb.create_sheet(title="Lists")
ws_lists["A1"] = "Not Started"
ws_lists["A2"] = "In Progress"
ws_lists["A3"] = "Completed"

# Set up Data Validation for the Status column (column D) in Tasks sheet
...
# Create the Summary sheet and add the metrics table
...
# Create a table for chart data (Completed vs. Not Completed)
...
# Create a pie chart for the project completion breakdown
pie = PieChart()
